...
if [ -z "${AUTH_USERNAME+set}" ]; then
  echo "Must set AUTH_USERNAME environment variable"
  exit 1
fi

if [ -z "${AUTH_PASSWORD+set}" ]; then
  echo "Must set AUTH_PASSWORD environment variable"
  exit 1
fi
...
cf set-env "${ROUTE_SERVICE_APP_NAME}" AUTH_USERNAME "${AUTH_USERNAME}"
